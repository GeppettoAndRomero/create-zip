...
export async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    // Service Workerを登録（/create-zip/sw.js, scope=/create-zip/）。
    // scope がツールの全 locale ページ（/create-zip/, /create-zip/ja/ …）を被覆する。
    const registration = await navigator.serviceWorker.register('/create-zip/sw.js', {
      scope: '/create-zip/',
    });
